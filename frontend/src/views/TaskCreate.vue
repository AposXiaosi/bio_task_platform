  ...
  <div class="task-create">
    <div class="create-heading">
      <div class="bio-page-header">
        <h1 class="bio-page-title">新建分析</h1>
        <p class="bio-page-subtitle">选择流程、上传数据并配置分析参数</p>
      </div>
      <div class="step-indicator"><span class="active">1</span><i /><span>2</span><i /><span>3</span></div>
    </div>
      ...
      >
        ...
        <template v-if="selectedType">
          ...
          <template v-if="selectedType === 'sequence_assembly'">
            <el-row :gutter="24">
              <el-col :span="8">
                <el-form-item label="最小重叠长度 (bp)" prop="params.min_overlap">
                  <el-input-number v-model="form.params.min_overlap" :min="3" :max="500" style="width: 100%" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
              ...
              <el-col :span="8">
                <el-form-item label="组装算法" prop="params.algorithm">
                  <el-select v-model="form.params.algorithm" style="width: 100%">
                    <el-option label="Greedy Overlap" value="greedy_overlap" />
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
...
function onTypeChange() {
  const defaults = {
    sequence_alignment: { matchScore: 2, mismatchPenalty: 2, gapPenalty: 3 },
    sequence_assembly: { min_overlap: 20, coverage: 30, algorithm: 'greedy_overlap' },
    gene_expression: { normalizationMethod: 'TPM' },
    diff_expression: { controlGroup: 'control', treatmentGroup: 'treatment', pValueThreshold: 0.05, foldChange: 2 },
    variant_calling: { minVariantQuality: 30, minDepth: 1 },
    functional_annotation: { eValueThreshold: -5, maxHits: 20 }
  }
  form.params = { ...(defaults[selectedType.value] || {}) }
}
...
.step-indicator { display: flex; align-items: center; gap: 7px; padding-top: 5px; }
.step-indicator span { width: 24px; height: 24px; display: grid; place-items: center; border: 1px solid #cfd8dc; border-radius: 50%; color: #829198; font-size: 10px; font-weight: 700; }
.step-indicator span.active { border-color: var(--bio-primary); background: var(--bio-primary); color: #fff; }
.step-indicator i { width: 22px; height: 1px; background: #d7dfe2; }
...
@media (max-width: 700px) { .step-indicator { display: none; } .bio-form-card :deep(.el-col) { max-width: 100%; flex: 0 0 100%; } }
</style>
