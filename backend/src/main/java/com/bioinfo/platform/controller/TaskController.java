...
public class TaskController {
    ...
    public ResponseEntity<ByteArrayResource> downloadTaskLogs(@PathVariable Long id) {
        List<TaskLogDTO> logs = taskService.getTaskLogs(id);
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS");
        StringBuilder content = new StringBuilder();
        for (TaskLogDTO item : logs) {
            content.append(item.getCreatedAt() == null ? "-" : item.getCreatedAt().format(formatter))
                    .append(" [").append(item.getLevel()).append("] ")
                    .append(item.getMessage()).append(System.lineSeparator());
        }
        ByteArrayResource resource = new ByteArrayResource(content.toString().getBytes(StandardCharsets.UTF_8));
        return ResponseEntity.ok()
                .contentType(MediaType.TEXT_PLAIN)
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=task-" + id + ".log")
